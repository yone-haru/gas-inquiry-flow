...
module.exports = [
  {
    ...
    rules: {
      // vars: "local" にすると、トップレベル（＝グローバルスコープ）の var/関数宣言は
      // 「未使用」チェックの対象から外れる。GASは全ファイルが1つのグローバルスコープを共有し、
      // トップレベルの宣言は基本的に他ファイルから参照される前提のため、これで
      // cross-file参照による誤検知（このファイル単体では未使用に見える）を避けつつ、
      // 関数内のローカル変数の本当の未使用は引き続き検出できる。
